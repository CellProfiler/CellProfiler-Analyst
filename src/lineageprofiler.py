...
class Timeline(object):
   ...
   def add_event(self, timepoint, action, well_ids):
      ...
      print set([well_id[0] for well_id in well_ids])
   ...
   def get_well_permutations(self, timepoint):
      ...
      '''
      d = {}
      for wellid in self.get_all_well_ids():
   ...
   def get_all_well_ids(self):
      '''returns unique well ids from all plates associated with this timeline.
      '''
      pw_ids = []
      for plate_id in self.plate_ids:
         pw_ids += [(plate_id, well_id) for well_id in PlateDesign.get_well_ids(PlateDesign.get_plate_format(plate_id))]
      return pw_ids
