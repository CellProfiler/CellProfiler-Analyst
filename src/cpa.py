...
class MainGUI(wx.Frame):
    ...
    def __init__(self, properties, parent, id=-1, **kwargs):
        ...
        self.pv_btn.SetPressColor(wx.Color(0,0,0))
        self.data_table_btn.SetPressColor(wx.Color(0,0,0))
        self.scatter_btn.SetPressColor(wx.Color(0,0,0))
        self.image_viewer_btn.SetPressColor(wx.Color(0,0,0))
        
        tb.AddControl(self.classifier_btn).SetShortHelp('Launch Classifier.')
        tb.AddControl(self.pv_btn).SetShortHelp('Launch the Plate Viewer tool.')
        tb.AddControl(self.data_table_btn).SetShortHelp('Launch the Data Table.')
        tb.AddControl(self.scatter_btn).SetShortHelp('Launch the Scatter Plot tool.')
        tb.AddControl(self.image_viewer_btn).SetShortHelp('Launch the Image Viewer tool.')
        self.tb.Realize()
...
#        hdlr.setFormatter(logging.Formatter('%(levelname)s | %(name)s | %(message)s [@ %(asctime)s in %(filename)s:%(lineno)d]'))
        ...
        self.Bind(wx.EVT_BUTTON, self.launch_classifier, self.classifier_btn)
        self.Bind(wx.EVT_BUTTON, self.launch_plate_map_browser, self.pv_btn)
        self.Bind(wx.EVT_BUTTON, self.launch_data_table, self.data_table_btn)
        self.Bind(wx.EVT_BUTTON, self.launch_scatter_plot, self.scatter_btn)
        self.Bind(wx.EVT_BUTTON, self.launch_image_viewer, self.image_viewer_btn)
