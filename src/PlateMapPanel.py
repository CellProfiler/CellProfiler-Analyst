...
class PlateMapPanel(wx.Panel):
    ...
    def GetWellAtCoord(self, x, y):
        ...
        r = min(self.Size[0]/(self.data.shape[1]+1.)/2.,
                self.Size[1]/(self.data.shape[0]+1.)/2.) - 1.
        i = int((x-2)/(r*2+2))
        j = int((y-2)/(r*2+2))
    ...
    def OnPaint(self, evt):
        ...
        py = 0
        i=0
        for y in range(self.data.shape[0]+1):
            px = 0
            for x in range(self.data.shape[1]+1):
                # Draw column headers
                if y==0 and x!=0:
                    dc.DrawText(self.col_labels[x-1], px+r/2, py+r/2)
                # Draw row headers
                elif y!=0 and x==0:
                    dc.DrawText(self.row_labels[y-1], px+r/2, py+r/2)
                # Draw wells
                elif y>0 and x>0:
                    if (x-1,y-1) in self.selection:
                        ...
                        dc.SetPen(wx.Pen("BLACK",min(r/10., 1.)))
                    color = np.array(self.colormap(self.data[y-1][x-1])[:3]) * 255
...
if __name__ == "__main__":
    ...
    p = PlateMapPanel(frame, data, shape=(16,24))
    frame.Show()
    
#    f = wx.Frame(None)
#    data = np.arange(96)/96.
#    p2 = PlateMapPanel(f, data, (8,12)) #96 wells
#    f.Show()
