...
class ExperimentSettingsWindow(wx.SplitterWindow):
    ...
    def onOpenWrokBench(self, event):
	pass
...
class OverviewPanel(wx.Panel):
	...
	for name, ctrl in self.settings_controls.items():
		...
		print name, ctrl.GetStringSelection()
		meta.set_field(name, ctrl.GetStringSelection())	
	    elif isinstance(ctrl, wx.DatePickerCtrl):
		selected = ctrl.GetValue()
		month = selected.Month + 1
		day = selected.Day
		year = selected.Year
		date = "%02d/%02d/%4d" % (day, month, year)	
		meta.set_field(name, date)
	    else:
...
class FCSPanel(wx.Panel):
	...
	if meta.get_field(unitTAG) is not None:
	    self.settings_controls[unitTAG].SetStringSelection(meta.get_field(unitTAG))
	self.settings_controls[fcsfcsfrmtTAGG].Bind(wx.EVT_CHOICE, self.OnSavingData)
